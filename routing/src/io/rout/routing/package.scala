...
package object routing extends ReqReads with Rout {
  ...
  implicit class FilterOps[A](val filter: Filter[Request, Response,A, Response]) extends AnyVal {
    ...
    def joinPath2[B,C](path: Path => Option[(B,Path)]) = new Filter[ReqReadExt[C], Response,Option[(C,A,B)], Response] {
      def apply(requestExt: ReqReadExt[C], service: Service[Option[(C,A,B)], Response]) =
        filter.andThen(a=>
  ...
  implicit class ReqReadOps[A](val rr: ReqRead[A]) extends AnyVal {
    ...
    def toFilter2[B] = new Filter[ReqReadExt[B], Response, (B,A), Response] {
      def apply(requestExt: ReqReadExt[B], service: Service[(B,A), Response]) =
