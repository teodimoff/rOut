...
trait RoutSpec extends FlatSpec with Matchers with Checkers with AllInstances
  ...
  def genOutputContext: Gen[(Status, Map[String, String], Seq[Cookie])] =
    for {
      s <- genStatus

    } yield (s, Map.empty[String, String], Seq.empty[Cookie])

  def genFailureOutput: Gen[Output.Failure] = for {
    (s, hs, cs) <- genOutputContext
    m <- Gen.alphaStr
  } yield Output.Failure(new Exception(m), Output.Meta(s, hs.toMap, cs.toSeq))

  def genPayloadOutput[A: Arbitrary]: Gen[Output.Payload[A]] = for {
    (s, hs, cs) <- genOutputContext
    a <- Arbitrary.arbitrary[A]
  } yield Output.Payload(a, Output.Meta(s, hs.toMap, cs.toSeq))
